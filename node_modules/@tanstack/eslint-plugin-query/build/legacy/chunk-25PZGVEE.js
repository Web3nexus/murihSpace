...
var ASTUtils = {
  ...
  traverseUpMemberExpression(node) {
    ...
    if (parent !== void 0 && (parent.type === AST_NODE_TYPES.MemberExpression && parent.object === node || parent.type === AST_NODE_TYPES.ChainExpression || parent.type === AST_NODE_TYPES.TSNonNullExpression)) {
  ...
  getExternalRefs(params) {
    ...
    const references = collectReferences(scope).filter((x) => x.isRead() && !scope.set.has(x.identifier.name)).map((x) => {
      ...
      const isComputedCallProperty = memberExpression !== void 0 && memberExpression.type === AST_NODE_TYPES.MemberExpression && memberExpression.computed && memberExpression.property === memberPath && memberExpression.parent.type === AST_NODE_TYPES.CallExpression && memberExpression.parent.callee === memberExpression;
...
var ExhaustiveDepsUtils = {
  isRelevantReference(params) {
    ...
    return reference.identifier.name !== "undefined" && !ExhaustiveDepsUtils.isFunctionCallTarget(reference.identifier) && reference.identifier.parent.type !== AST_NODE_TYPES2.NewExpression && !ExhaustiveDepsUtils.isInstanceOfKind(reference.identifier.parent);
  },
  isFunctionCallTarget(identifier) {
    const callee = ASTUtils.traverseUpMemberExpression(identifier);
    return callee.parent !== void 0 && callee.parent.type === AST_NODE_TYPES2.CallExpression && callee.parent.callee === callee;
  },
  /**
...
var rule3 = createRule3({
  ...
  create: detectTanstackQueryImports((context, _, helpers) => {
    ...
    return {
      CallExpression: (node) => {
        if (node.parent.type !== AST_NODE_TYPES6.VariableDeclarator) {
          return;
        }
        ...
        if (!isDirectHook) {
          const canReportQueryResult = returnValue.type === AST_NODE_TYPES6.Identifier || NoRestDestructuringUtils.isObjectRestDestructuring(returnValue);
...
var rule4 = createRule4({
  ...
  create: detectTanstackQueryImports((context, _options, helpers) => {
    ...
    function getDirectQueryHook(callExpression) {
      if (callExpression.callee.type !== AST_NODE_TYPES7.Identifier || !allHookNames.includes(callExpression.callee.name) || !helpers.isTanstackQueryImport(callExpression.callee)) {
        return void 0;
      }
    ...
    function getTrackedQueryHook(callExpression) {
      ...
      if (callExpression.callee.type === AST_NODE_TYPES7.Identifier) {
    ...
    function getReturnedQueryHook(body) {
      var _a;
      if (body.type === AST_NODE_TYPES7.CallExpression) {
        return getDirectQueryHook(body);
      }
      if (body.type !== AST_NODE_TYPES7.BlockStatement) {
        return void 0;
      }
      const returnStatements = body.body.filter(
        (statement) => statement.type === AST_NODE_TYPES7.ReturnStatement
      );
      if (returnStatements.length !== 1) {
        return void 0;
      }
      const returnArgument = (_a = returnStatements[0]) == null ? void 0 : _a.argument;
      if ((returnArgument == null ? void 0 : returnArgument.type) === AST_NODE_TYPES7.CallExpression) {
    ...
    return {
      ...
      VariableDeclarator(node) {
        if (node.id.type === AST_NODE_TYPES7.Identifier && isCustomHookName(node.id.name) && node.init !== null && (node.init.type === AST_NODE_TYPES7.ArrowFunctionExpression || node.init.type === AST_NODE_TYPES7.FunctionExpression)) {
          const queryHook = getReturnedQueryHook(node.init.body);
          if (queryHook !== void 0) {
            trackedCustomHooks[node.id.name] = queryHook;
          }
        }
        if (node.init !== null && node.init.type === AST_NODE_TYPES7.CallExpression) {
      ...
      "Program:exit"() {
        pendingVariableDeclarators.forEach((node) => {
          var _a;
          if (((_a = node.init) == null ? void 0 : _a.type) !== AST_NODE_TYPES7.CallExpression) {
            return;
          }
...
//# sourceMappingURL=chunk-25PZGVEE.js.map