...
function SectionCard({ title, icon: Icon, description, children }: { title: string; icon: React.ElementType; description?: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border-none/50 bg-card/50 backdrop-blur-xl  overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="p-4 border-b border-border/50 flex items-start gap-4">
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 p-4 lg:p-5 animate-in fade-in duration-500 pb-24">
      <PageSecondaryNav
        ...
        subtitle="Build your professional portfolio. Share your audience stats, past work, and rates."
          ...
          <div className="flex items-center gap-2">
            {kit.is_published && creatorId && (
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="px-4 py-2 rounded-lg bg-background border border-border text-foreground text-xs font-bold hover:border-primary/50 transition-all flex items-center gap-1.5 group"
              >
                <ExternalLink weight="fill" className="w-3.5 h-3.5 text-muted-foreground group-hover:text-primary transition-colors" />
                View Public Kit
              </a>
            )}
            ...
            <button 
              onClick={saveKit} 
              disabled={saving}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#2164b6] hover:bg-[#1a5091] text-white text-xs font-bold transition-all shadow-xs disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <SectionCard title="About You" icon={FileText} description="Write a compelling bio to introduce yourself to brands.">
            <textarea 
              ...
              rows={4} 
              className="w-full rounded-lg bg-background border-none/50 p-4 text-sm font-medium focus:ring-2 focus:ring-[#2164b6]/50 resize-none" 
              ...
              {[
                ...
                <div key={f.key} className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full h-12 rounded-lg bg-background border-none/50 px-4 text-sm font-medium focus:ring-2 focus:ring-[#2164b6]/50"
                  />
                  {f.sug != null && f.sug > 0 && (
            ...
            <div className="pt-6 mt-6 border-t border-border/50">
              ...
              <div className="flex gap-3 mb-4">
                <input 
                  ...
                  className="flex-1 h-12 rounded-lg bg-background border-none/50 px-4 text-sm focus:ring-[#2164b6]/50"
                ...
                <input 
                  ...
                  placeholder="%" 
                  className="w-24 shrink-0 h-12 rounded-lg bg-background border-none/50 px-4 text-sm focus:ring-[#2164b6]/50"
                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addDemographic(); } }}
                />
                <button type="button" onClick={addDemographic} className="h-12 px-6 rounded-lg bg-muted text-foreground font-bold hover:bg-muted/80 transition-colors">
                  Add
                </button>
              </div>
              ...
              {kit.audience_demographics && Object.keys(kit.audience_demographics).length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {Object.entries(kit.audience_demographics).map(([k, v]) => (
                    <div key={k} className="flex items-center gap-2 pl-3 pr-1.5 py-1.5 rounded-lg bg-background border-none/50">
                      ...
                      >
                        <Trash2 weight="fill" className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
                </div>
          ...
          <SectionCard title="Past Partnerships" icon={Briefcase} description="Build trust by showing brands you've worked with before.">
            <div className="flex gap-3 mb-4">
              ...
              />
              <button type="button" onClick={addPartnership} className="h-12 px-6 rounded-lg bg-muted text-foreground font-bold hover:bg-muted/80 transition-colors">
                Add
              </button>
            </div>
            ...
            {kit.past_partnerships.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {kit.past_partnerships.map((p, i) => (
                  <div key={i} className="flex items-center gap-2 pl-3 pr-1.5 py-1.5 rounded-lg bg-background border-none/50">
        ...
        <div className="space-y-8">
          ...
          <SectionCard title="Visibility" icon={Globe}>
            <label className="flex items-center justify-between gap-4 p-4 rounded-lg border-none/50 bg-background cursor-pointer group">
              <div>
          ...
          <SectionCard title="Profile Photo" icon={ImageIcon}>
            <div className="bg-background rounded-lg border-none/50 p-4">
               <ImageUploader
          ...
          <SectionCard title="Rate Card" icon={DollarSign} description="What you charge for your services.">
            <div className="space-y-3 mb-6">
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 h-10 rounded-lg bg-background border-none/50 px-3 text-sm focus:ring-[#2164b6]/50"
            ...
            <div className="space-y-2">
              {kit.rate_card.length === 0 ? (
                <div className="p-4 text-center border border-dashed border-border/50 rounded-lg bg-background/50">
                  <p className="text-xs font-medium text-muted-foreground">No rates added yet.</p>
                </div>
              ) : (
                kit.rate_card.map((r, i) => (
                  <div key={i} className="flex items-center justify-between gap-3 p-3 rounded-lg bg-background border-none/50 hover:border-primary/30 transition-colors group">
