    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 p-4 lg:p-5">
      <div>
        <h1 className="text-xl font-black tracking-tight flex items-center gap-2.5">
          <BarChart3 weight="fill" className="h-6 w-6 text-[#2164b6] dark:text-[#7ab0ff]" /> Product Performance
        </h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="border-none rounded-lg bg-card p-5">
          ...
          <p className="text-xl font-black text-foreground mt-1">{stats[0]?.currency ?? 'USD'} {totalRevenue.toLocaleString()}</p>
        </div>
        <div className="border-none rounded-lg bg-card p-5">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Total Sales</p>
          <p className="text-xl font-black text-foreground mt-1">{totalSales}</p>
        </div>
      </div>

      {stats.length === 0 ? (
        <div className="p-16 text-center border border-dashed border-border rounded-3xl bg-card">
          <BarChart3 weight="fill" className="h-12 w-12 text-muted-foreground/30 mx-auto mb-3" />
                ...
                {stats.map((s) => (
                  <tr key={s.product_id} className="hover:bg-muted/10 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {s.trend === 'up' ? <TrendingUp weight="fill" className="h-4 w-4 text-emerald-400" /> : s.trend === 'down' ? <TrendDown weight="fill" className="h-4 w-4 text-rose-400" /> : <Minus weight="fill" className="h-4 w-4 text-muted-foreground" />}
                    </td>
                  </tr>
                ))}
              </tbody>
