...
export function EscrowPage() {
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto p-4 lg:p-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-foreground tracking-tight flex items-center gap-2.5">
            <ShieldCheck weight="fill" className="h-6 w-6 text-secondary" />
            Escrow Balances
          </h1>
      ...
      {isLoading ? (
        <div className="py-20 text-center"><Loader2 weight="fill" className="h-6 w-6 animate-spin text-secondary mx-auto" /></div>
        ...
        <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-border rounded-3xl bg-card space-y-3">
          <ShieldCheck weight="fill" className="h-10 w-10 text-muted-foreground/30 mx-auto" />
          ...
          {escrows.map((escrow) => {
            ...
            return (
              <div
                ...
                className="rounded-lg border-none bg-card p-4 flex items-center justify-between gap-4 cursor-pointer hover:bg-muted/30 transition-colors"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border-none bg-card p-4 space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="text-xl font-extrabold text-foreground">{formatPrice(selectedEscrow.amount, selectedEscrow.currency)}</span>
            </div>
            ...
            {selectedEscrow.status === 'held' && (
              <div className="flex items-center gap-2 pt-2">
                <button onClick={() => handleAction('release', selectedEscrow.id)} className="px-4 py-2 rounded-lg bg-emerald-500 text-white font-bold text-xs hover:bg-emerald-600 flex items-center gap-1.5">
                  <Check weight="fill" className="h-3.5 w-3.5" /> Release to Seller
                </button>
                <button onClick={() => { setShowDisputeForm(true); setDisputeReason(''); }} className="px-4 py-2 rounded-lg bg-red-500/10 text-red-600 font-bold text-xs hover:bg-red-500/20 border border-red-500/30 flex items-center gap-1.5">
                  <ShieldWarning weight="fill" className="h-3.5 w-3.5" /> Open Dispute
                </button>
              </div>
            )}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedEscrow.disputes.map((d) => {
                    ...
                    return (
                      <div key={d.id} className="rounded-lg border-none bg-muted/20 p-3 space-y-1.5">
      ...
      {showDisputeForm && selectedEscrow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="border-none rounded-lg bg-card p-4 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              <button onClick={() => setShowDisputeForm(false)} className="px-4 py-2 text-xs font-semibold rounded-lg hover:bg-muted transition-colors">Cancel</button>
              <button onClick={() => handleOpenDispute(selectedEscrow.id)} className="px-4 py-2 text-xs font-bold rounded-lg bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-all">
                Submit Dispute
              </button>
            </div>
          </div>
