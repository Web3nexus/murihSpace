    ...
    <div className="space-y-6 w-full max-w-7xl mx-auto p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold">Cart ({cart.item_count} items)</h1>
        </div>
        <button
          onClick={clearCart}
        ...
        {cart.items.map((item) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-medium truncate">{item.product?.title ?? 'Unknown product'}</h3>
              <p className="text-sm text-gray-500">by @{item.product?.creator?.username}</p>
