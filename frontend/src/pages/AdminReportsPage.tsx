...
export function AdminReportsPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) { const j = await res.json(); setLogs(j?.data?.data ?? j?.data ?? []); }
    ...
    <div className="w-full mx-auto max-w-[1400px] space-y-6 p-6 lg:p-10">
      <div>
        <h1 className="text-2xl font-black tracking-tight flex items-center gap-2.5">
          <FileText className="h-6 w-6 text-[#38A8D8]" /> Reports & Audit
        </h1>
      ...
      <div className="flex flex-wrap gap-1">
        {actions.map((a) => (
          <button key={a} onClick={() => setActionFilter(a)} className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition-colors ${actionFilter === a ? 'bg-[#38A8D8] text-foreground' : 'bg-muted text-muted-foreground hover:text-foreground'}`}>
            {a ? a.split('.').pop() : 'All'}
          </button>
        ))}
      </div>
      ...
      {loading ? <div className="flex justify-center py-12">  <Loader2 className="h-8 w-8 animate-spin text-[#38A8D8]" /></div>
        : logs.length === 0 ? <div className="p-12 text-center border border-dashed border-border rounded-3xl bg-card"><FileText className="h-10 w-10 text-muted-foreground/30 mx-auto mb-3" /><h3 className="text-sm font-bold text-foreground/60">No audit logs</h3></div>
              ...
              {logs.map((l) => (
                <div key={l.id} className="flex items-center justify-between px-4 py-3 hover:bg-muted/10 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <Flag className="h-4 w-4 text-[#38A8D8] shrink-0" />
