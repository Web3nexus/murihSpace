...
const CATEGORY_ICONS: Record<string, React.ReactNode> = {
  ...
  store: <ShoppingBag weight="fill" className="h-4.5 w-4.5" />,
  subscriptions: <Crown weight="fill" className="h-4.5 w-4.5" />,
  security: <ShieldCheck weight="fill" className="h-4.5 w-4.5" />,
};

function categoryIcon(id: string) {
  return CATEGORY_ICONS[id] ?? <HelpCircle weight="fill" className="h-4.5 w-4.5" />;
}
    ...
    <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200/80 dark:border-slate-800/80 bg-white/85 dark:bg-slate-950/85 backdrop-blur-md">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-3 flex items-center gap-3">
          ...
          >
            <Menu weight="fill" className="h-5 w-5" />
          </button>
          ...
          {isAuthenticated ? (
            ...
            <Link
              to="/login"
              className="px-4 py-2 rounded-lg bg-[#2164b6] hover:bg-[#1a5091] text-white text-xs font-bold transition-colors "
            >
              Log in
            </Link>
          )}
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#2164b6] via-[#2a76cf] to-[#2164b6] dark:from-[#12356b] dark:via-[#1a4d8f] dark:to-[#2a6a9e]">
        ...
        <div className="relative max-w-[1400px] mx-auto px-4 sm:px-6 py-10 sm:py-14 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 text-white/90 text-[11px] font-semibold mb-4 backdrop-blur-sm">
            <BookOpen weight="fill" className="h-3.5 w-3.5" /> How can we help you today?
          </div>
          <h1 className="text-xl sm:text-xl font-black tracking-tight text-white">MurihSpace Help Center</h1>
          ...
          <div className="mt-6 max-w-xl mx-auto">
            <div className="flex items-center gap-2 bg-white dark:bg-slate-900 rounded-lg px-4 py-3 shadow-lg shadow-black/10 focus-within:ring-4 focus-within:ring-white/20 transition-all">
              <Search weight="fill" className="h-5 w-5 text-slate-400 shrink-0" />
              <input
                value={query}
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 grid grid-cols-1 lg:grid-cols-[240px_1fr_340px] gap-4">
        {/* Left nav */}
        <aside
        ...
        >
          ...
          >
            <BookOpen weight="fill" className="h-4.5 w-4.5" /> All topics
          </button>
              ...
              return (
                <div key={cat.id} className="rounded-lg">
                  ...
                  >
                    ...
                    <ChevronRight weight="fill" className={`h-3.5 w-3.5 transition-transform ${active ? "rotate-90" : "text-slate-400"}`} />
                  </button>
                  {active && (
          ...
          {query.trim().length >= 2 ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-sm font-black">
                <Search weight="fill" className="h-4 w-4 text-[#2164b6] dark:text-[#7ab0ff]" />
                Search results
              ...
              {searchResults.length === 0 ? (
                <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-10 text-center">
                  <Search weight="fill" className="h-8 w-8 mx-auto mb-3 text-slate-300 dark:text-slate-600" />
                ...
                searchResults.map((a) => {
                  ...
                  return (
                    <button
                      key={a.id}
                      onClick={() => openArticle(a.id)}
                      className="w-full text-left rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 hover:border-[#2164b6]/40 hover: transition-all group"
                    >
        ...
        <aside className="lg:sticky lg:top-[72px] lg:h-[calc(100vh-90px)]">
          <div className="flex flex-col h-[480px] sm:h-[560px] lg:h-full rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900  overflow-hidden">
            {/* AI header */}
            <div className="shrink-0 px-4 py-3.5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-[#2164b6]/5 to-[#2164b6]/5">
              <div className="flex items-center gap-2.5">
                <div className="relative">
                  <div className="h-9 w-9 rounded-lg bg-gradient-to-br from-[#2164b6] to-[#2164b6] flex items-center justify-center ">
              ...
              <button
                ...
                aria-label="New chat"
              >
                <RefreshCw weight="fill" className="h-3.5 w-3.5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col">
                  <div className="flex items-start gap-2.5">
                    <div className="h-8 w-8 shrink-0 rounded-lg bg-gradient-to-br from-[#2164b6] to-[#2164b6] flex items-center justify-center">
                      <MeraIcon className="h-4 w-4" />
                    </div>
                    <div className="bg-slate-100 dark:bg-slate-800 rounded-lg rounded-tl-sm px-3.5 py-2.5 text-xs leading-relaxed text-slate-600 dark:text-slate-300">
                  ...
                  {!isAuthenticated && (
                    <div className="mt-auto pt-4">
                      <div className="rounded-lg border border-[#2164b6]/20 bg-[#2164b6]/5 p-4">
                        ...
                        <Link to="/login" className="mt-3 inline-flex px-4 py-2 rounded-lg bg-[#2164b6] hover:bg-[#1a5091] text-white text-[11px] font-bold transition-colors">
                          Log in to chat
                        </Link>
                      </div>
                    </div>
                  ...
                  <div key={i} className={`flex gap-2.5 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                    ...
                    <div className={`max-w-[80%] ${m.role === "user" ? "order-first" : ""}`}>
                      <div className={`px-3.5 py-2.5 text-xs leading-relaxed whitespace-pre-wrap break-words ${
                        m.role === "assistant"
                          ? "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-lg rounded-tl-sm"
                          : "bg-[#2164b6] text-white rounded-lg rounded-tr-sm"
                      }`}>
                        {m.content}
                      </div>
                        ...
                        <div className="mt-1.5 flex flex-wrap gap-1">
                          {m.sources.map((sid) => {
                            ...
                            return (
                              ...
                              >
                                <ArrowUpRight weight="fill" className="h-3 w-3" /> {a.title}
                              </button>
                            );
                          })}
                        </div>
              ...
              {busy && (
                <div className="flex gap-2.5">
                  ...
                  <div className="bg-slate-100 dark:bg-slate-800 rounded-lg rounded-tl-sm px-3 py-2">
                    <TypingDots />
                  </div>
                </div>
              )}
            ...
            <div className="shrink-0 p-3 border-t border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 rounded-lg px-3 py-2">
                ...
                >
                  {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send weight="fill" className="h-3.5 w-3.5" />}
                </button>
              </div>
          ...
          {popular.map((id) => {
            ...
            return (
              <button
                ...
                className="text-left rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 hover:border-[#2164b6]/40 hover: transition-all group"
              >
        ...
        <div className="mt-3 grid sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {categories.map((cat) => (
            <button
              ...
              className="text-left rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 hover:border-[#2164b6]/40 hover: hover:-translate-y-0.5 transition-all group"
            >
              <div className="h-9 w-9 rounded-lg bg-[#2164b6]/10 text-[#2164b6] dark:text-[#7ab0ff] flex items-center justify-center group-hover:scale-105 transition-transform">
                {categoryIcon(cat.id)}
              </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-3">
        <div className="h-11 w-11 rounded-lg bg-[#2164b6]/10 text-[#2164b6] dark:text-[#7ab0ff] flex items-center justify-center">
      ...
      <div className="mt-4 space-y-2">
        {list.map((a) => (
          <button
            ...
            className="w-full text-left rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 hover:border-[#2164b6]/40 hover: transition-all group"
          >
  ...
  return (
    <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
      <div className="border-b border-slate-200 dark:border-slate-800 px-5 sm:px-8 py-4 flex items-center justify-between gap-3">
        ...
        >
          <ChevronLeft weight="fill" className="h-3.5 w-3.5" /> All topics
        </button>
        <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wide text-[#2164b6] dark:text-[#7ab0ff]">
      ...
      <div className="px-5 sm:px-8 py-6 sm:py-8">
        <h1 className="text-xl sm:text-xl font-black tracking-tight leading-snug">{article.title}</h1>
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{article.excerpt}</p>

        {/* Inline TOC */}
        <button
          onClick={() => setTocOpen((v) => !v)}
          className="mt-4 w-full flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:border-[#2164b6]/40 transition-colors"
        >
          In this article
          <ChevronDown weight="fill" className={`h-3.5 w-3.5 transition-transform ${tocOpen ? "rotate-180" : ""}`} />
        </button>
        {tocOpen && (
          <div className="mt-1 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-2">
            ...
            <div className="mt-2.5 flex flex-wrap gap-2">
              {article.related.map((rid) => {
                ...
                return (
                  ...
                  >
                    <ExternalLink weight="fill" className="h-3 w-3" /> {r.title}
                  </button>
                );
              })}
            </div>
