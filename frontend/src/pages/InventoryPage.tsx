    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 p-4 lg:p-5">
      <div>
        <h1 className="text-xl font-black tracking-tight flex items-center gap-2.5">
          <Package weight="fill" className="h-6 w-6 text-[#2164b6] dark:text-[#7ab0ff]" /> Inventory Management
        </h1>
      ...
      {items.length === 0 ? (
        <div className="p-16 text-center border border-dashed border-border rounded-3xl bg-card">
          <Package weight="fill" className="h-12 w-12 text-muted-foreground/30 mx-auto mb-3" />
                      ...
                      <td className="px-4 py-3">
                        {low ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-500/20 text-rose-400">
                            <AlertTriangle weight="fill" className="h-3 w-3" /> Low Stock
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3">
                        {editing?.id === item.id ? (
                          ...
                          <Button size="sm" variant="ghost" className="h-7 text-[10px]" onClick={() => { setEditing(item); setEditQty(String(item.quantity)); }}>
                            <Edit weight="fill" className="h-3 w-3" />
                          </Button>
                        )}
                      </td>
                    </tr>
