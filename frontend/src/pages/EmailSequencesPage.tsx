  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 p-4 lg:p-5 animate-in fade-in duration-500 pb-24">
      <PageSecondaryNav
        ...
        subtitle="Create email drip campaigns that automatically trigger when users take specific actions."
        icon={<ListNumbers weight="fill" className="h-5 w-5 text-primary" />}
        tabs={marketingTabs}
        actions={
          <button 
        ...
        <div className="rounded-lg border-none/50 bg-card/50 backdrop-blur-xl shadow-xl overflow-hidden animate-in slide-in-from-top-4 duration-500">
          <div className="p-4 border-b border-border/50 flex items-center gap-4 bg-background/50">
            <div className="p-2.5 rounded-lg bg-[#2164b6]/10 text-[#2164b6] shrink-0">
              <SlidersHorizontal weight="fill" className="w-5 h-5" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2 flex flex-col h-full">
                ...
                <textarea 
                  ...
                  className="w-full flex-1 rounded-lg bg-background border-none/50 p-4 text-sm font-medium focus:ring-2 focus:ring-[#2164b6]/50 transition-all resize-none min-h-[120px]" 
                />
              </div>
            </div>
      ...
      {sequences.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center gap-4 py-24 rounded-3xl border border-dashed border-border/50 bg-card/30 backdrop-blur-sm">
          <div className="w-20 h-20 rounded-3xl bg-[#2164b6]/10 flex items-center justify-center mb-2">
            <ListNumbers weight="fill" className="h-10 w-10 text-[#2164b6]" />
          </div>
          <h2 className="text-xl font-bold text-foreground tracking-tight">No sequences yet</h2>
        ...
        <div className="space-y-6">
          {sequences.map(s => (
            <div key={s.id} className={`rounded-lg border ${s.is_active ? 'border-emerald-500/30' : 'border-border/50'} bg-card/50 backdrop-blur-xl overflow-hidden transition-all duration-300`}>
              
              {/* Sequence Header Card */}
              <div className="p-5 md:p-4 flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wider bg-background border-none/50 text-muted-foreground flex items-center gap-1">
                      <Play weight="fill" className="w-3 h-3" /> Trigger: {triggerLabel(s.trigger_event)}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0 border-t md:border-t-0 border-border/50 pt-4 md:pt-0">
                  ...
                  <button 
                    ...
                    className={`h-10 px-4 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${
                      s.is_active 
                        ? 'bg-amber-500/10 text-amber-500 hover:bg-amber-500 hover:text-white' 
                        : 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-white'
                    }`}
                  >
                    {s.is_active ? <><Power className="w-4 h-4" /> Pause</> : <><Power weight="fill" className="w-4 h-4" /> Activate</>}
                  </button>
                  <button 
              ...
              {expandedId === s.id && (
                <div className="bg-background/80 border-t border-border/50 p-4 md:p-5 animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className="max-w-4xl mx-auto space-y-6">
                    
                    <div className="flex justify-center">
                      <div className="px-6 py-3 rounded-full bg-muted border-none/50 text-sm font-bold text-foreground flex items-center gap-2 ">
                        <Play weight="fill" className="w-4 h-4 text-primary" /> User triggers '{triggerLabel(s.trigger_event)}'
                      </div>
                    </div>
                    
                    {(s.steps?.length ?? 0) === 0 ? (
                      <div className="flex flex-col items-center py-8 opacity-50">
                        <ArrowDown weight="fill" className="w-6 h-6 text-muted-foreground mb-4" />
                      ...
                      <div className="space-y-6 relative">
                        ...
                        {(s.steps ?? []).map((st, idx) => (
                          <div key={st.id} className="relative z-10 flex flex-col items-center">
                            ...
                            {/* Delay Node */}
                            <div className="bg-background border-none/50 rounded-full px-4 py-1.5 mb-6 text-[10px] font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1.5 ">
                              <Clock weight="fill" className="w-3.5 h-3.5" /> 
                              Wait {st.delay_days} {st.delay_days === 1 ? 'day' : 'days'}
                            </div>
                            
                            {/* Email Card */}
                            <div className="w-full max-w-2xl bg-card rounded-lg border-none/50  overflow-hidden group transition-all hover:border-primary/30">
                              <div className="p-4 flex items-start gap-4">
                                <div className="w-10 h-10 shrink-0 rounded-lg bg-blue-500/10 text-blue-500 flex items-center justify-center font-black text-lg">
                                  {idx + 1}
                                </div>
                    ...
                    <div className="relative z-10 flex flex-col items-center pt-6">
                      <div className="bg-background border-none/50 rounded-full px-4 py-1.5 mb-6 text-[10px] font-black uppercase tracking-wider text-muted-foreground flex items-center gap-1.5 ">
                        <Plus weight="fill" className="w-3.5 h-3.5" /> Next Step
                      </div>
                      
                      <div className="w-full max-w-2xl bg-card/30 rounded-lg border border-dashed border-border/80 p-5 space-y-4">
                        <div className="flex items-center gap-4">
                          <div className="w-24 shrink-0 space-y-1.5">
                            ...
                            <input 
                              ...
                              className="w-full h-10 rounded-lg bg-background border-none/50 px-3 text-sm focus:ring-[#2164b6]/50" 
                            />
                          </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <textarea 
                            ...
                            rows={3}
                            className="w-full rounded-lg bg-background border-none/50 p-3 text-sm focus:ring-[#2164b6]/50 resize-none font-mono" 
                          />
                        </div>
                        <div className="flex justify-end pt-2">
                          ...
                          >
                            <Send weight="fill" className="w-3.5 h-3.5" /> Add Email to Flow
                          </button>
                        </div>
                      </div>
                    </div>
