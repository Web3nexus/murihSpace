  ...
  if (loading || !wallet) {
    return (
      <div className="flex items-center justify-center h-64 text-muted-foreground">
        <RefreshCw weight="fill" className="h-6 w-6 animate-spin mr-2" /> Loading Business Wallet...
      </div>
    );
  }
    ...
    <div className="p-4 lg:p-5 space-y-6 max-w-7xl mx-auto w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
            <Building2 weight="fill" className="h-6 w-6 text-emerald-500" /> Business Revenue Wallet
          </h1>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm shadow transition"
        >
      ...
      <div className="p-4 rounded-lg border-none bg-card space-y-4 ">
        <h2 className="text-base font-bold text-foreground flex items-center gap-2">
          <Clock weight="fill" className="h-5 w-5 text-emerald-500" /> Storefront Earnings & Internal Transfers
        </h2>
        <p className="text-sm text-muted-foreground">
          Storefront order funds are held safely in <strong className="text-foreground">Escrow</strong> during fulfillment. Once orders are completed, funds move to <strong className="text-foreground">Available</strong> balance.
