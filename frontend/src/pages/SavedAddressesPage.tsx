    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 p-4 lg:p-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black tracking-tight flex items-center gap-2.5">
            <MapPin weight="fill" className="h-6 w-6 text-[#2164b6] dark:text-[#7ab0ff]" /> Saved Addresses
          </h1>
        ...
        <Button onClick={() => { resetForm(); setShowForm(true); }} className="text-sm font-bold gap-1.5">
          <Plus weight="fill" className="h-4 w-4" /> Add Address
        </Button>
      </div>
        ...
        <form onSubmit={handleSave} className="border-none rounded-lg bg-card p-4 space-y-4">
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={saving} className="text-sm font-bold">
              {saving ? <Loader2 weight="fill" className="h-4 w-4 animate-spin mr-2" /> : null}{editing ? "Update" : "Save"}
            </Button>
      ...
      {addresses.length === 0 ? (
        <div className="p-16 text-center border border-dashed border-border rounded-3xl bg-card">
          <MapPin weight="fill" className="h-12 w-12 text-muted-foreground/30 mx-auto mb-3" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {addresses.map((a) => (
            <div key={a.id} className="border-none rounded-lg bg-card p-5 space-y-3 hover: transition-shadow relative">
              {a.is_default && <span className="absolute top-3 right-3 text-emerald-400"><CheckCircle weight="fill" className="h-4 w-4" /></span>}
              <div className="flex items-center gap-2">
                <MapPin weight="fill" className="h-4 w-4 text-[#2164b6] dark:text-[#7ab0ff]" />
              ...
              <div className="flex gap-1 pt-2">
                ...
                <Button size="sm" variant="ghost" className="h-7 text-[10px]" onClick={() => {
                  ...
                  setShowForm(true);
                  setMsg(null);
                }}><Edit weight="fill" className="h-3 w-3" /></Button>
                <Button size="sm" variant="ghost" className="h-7 text-[10px] text-destructive" onClick={() => handleDelete(a.id)}><Trash2 weight="fill" className="h-3 w-3" /></Button>
              </div>
            </div>
          ))}
        </div>
