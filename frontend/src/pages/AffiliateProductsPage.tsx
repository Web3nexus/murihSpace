...
export default function AffiliateProductsPage() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-8 p-4 lg:p-5 animate-in fade-in duration-500 pb-24">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-20">
        ...
        <button 
          ...
          className={`flex items-center justify-center gap-2 px-6 py-3 rounded-lg text-sm font-bold shadow-lg transition-all duration-300 ${
            showForm 
      ...
      {/* Summary Cards */}
      {products.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-4 animate-in slide-in-from-bottom-4 duration-500">
          <StatCard icon={ShoppingBag} label="Active Links" value={String(products.length)} color="bg-blue-500 text-blue-500" />
          <StatCard icon={CursorClick} label="Total Clicks" value={totalClicks.toLocaleString()} color="bg-amber-500 text-amber-500" />
        ...
        <div className="rounded-lg border-none/50 bg-card/50 backdrop-blur-xl shadow-xl overflow-hidden animate-in slide-in-from-top-4 duration-500">
          <div className="p-4 border-b border-border/50 flex items-center gap-4 bg-background/50">
            <div className="p-2.5 rounded-lg bg-[#2164b6]/10 text-[#2164b6] shrink-0">
              <Link weight="fill" className="w-5 h-5" />
            </div>
            <div>
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center gap-4 py-24 rounded-3xl border border-dashed border-border/50 bg-card/30 backdrop-blur-sm">
          <div className="w-20 h-20 rounded-3xl bg-[#2164b6]/10 flex items-center justify-center mb-2">
            <Link weight="fill" className="h-10 w-10 text-[#2164b6]" />
          </div>
          <h2 className="text-xl font-bold text-foreground tracking-tight">No products yet</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {products.map((p) => {
            ...
            return (
              <div key={p.id} className={`group relative rounded-lg border ${p.is_active ? 'border-border/50 hover:border-primary/30' : 'border-border/30 opacity-75'} bg-card/50 backdrop-blur-xl p-5 md:p-4 transition-all duration-300 flex flex-col h-full`}>
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <a href={p.url} target="_blank" rel="noreferrer" className="text-xs text-muted-foreground hover:text-primary transition-colors flex items-center gap-1 w-fit max-w-full">
                      ...
                      <ExternalLink weight="fill" className="w-3 h-3 shrink-0" />
                    </a>
                  </div>
                  
                  <div className="shrink-0 flex items-center justify-center w-12 h-12 rounded-lg bg-emerald-500/10 text-emerald-500 flex-col group-hover:scale-105 transition-transform">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6 mt-auto">
                  <div className="bg-background rounded-lg border-none/50 p-3 space-y-1">
                  ...
                  <div className="bg-background rounded-lg border-none/50 p-3 space-y-1">
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      <BarChart3 weight="fill" className="w-3.5 h-3.5" />
                  ...
                  <div className="bg-background rounded-lg border-none/50 p-3 space-y-1">
                    <div className="flex items-center gap-1.5 text-muted-foreground">
                      <DollarSign weight="fill" className="w-3.5 h-3.5" />
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border/50 mt-auto">
                  <button 
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${
                      copiedId === p.id 
                        ...
                        : 'bg-background border-none/50 text-foreground hover:bg-muted'
                    }`}
                  >
                    {copiedId === p.id ? <Check className="w-3.5 h-3.5" /> : <Copy weight="fill" className="w-3.5 h-3.5" />}
                  ...
                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    >
                      {p.is_active ? <ToggleRight className="w-4 h-4 text-emerald-500" /> : <ToggleLeft weight="fill" className="w-4 h-4" />}
                    </button>
                    <button 
                      onClick={() => openEdit(p)} 
                      className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
                      title="Edit Product"
                    >
                      <Pencil weight="fill" className="w-4 h-4" />
                    </button>
                    <button 
