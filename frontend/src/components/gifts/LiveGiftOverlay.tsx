...
export function LiveGiftOverlay({ giftEvent, onAnimationComplete }: LiveGiftOverlayProps) {
  ...
  return (
    ...
    >
      <div
        className={`animate-bounce-in flex items-center gap-3 px-6 py-4 rounded-lg shadow-2xl border backdrop-blur-md ${
          isPremium
        ...
        <div className="relative">
          <div className="w-12 h-12 rounded-full bg-primary/20 border border-primary flex items-center justify-center overflow-hidden">
            {activeGift.sender.avatar ? (
              ...
              <Gift weight="fill" className="h-6 w-6 text-primary" />
            )}
          </div>
          {isPremium && (
            <Sparkle weight="fill" className="h-5 w-5 text-amber-400 absolute -top-1 -right-1 animate-spin" />
          )}
        </div>
