              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground/80 truncate mt-0.5">
                    {c.latest_message
                      ? (() => { const isCall = c.latest_message.type === 'call' || (typeof c.latest_message.content === 'string' && (c.latest_message.content.trim().startsWith('{"call_id"') || (c.latest_message.content.trim().startsWith('{') && c.latest_message.content.includes('"call_id"')))); if (isCall) { try { const d = JSON.parse(c.latest_message.content); const missed = d.status === 'missed' || d.status === 'declined'; if (missed) return d.call_type === 'video' ? '📹 Missed Video Call' : '📞 Missed Call'; if (d.duration > 0) { const m = Math.floor(d.duration / 60); const s = d.duration % 60; return d.call_type === 'video' ? `📹 Video Call · ${m > 0 ? m + 'm ' : ''}${s}s` : `📞 Voice Call · ${m > 0 ? m + 'm ' : ''}${s}s`; } return d.call_type === 'video' ? '📹 Video Call' : '📞 Voice Call'; } catch { return '📞 Call'; } } return c.latest_message.content; })()
                      : "No messages yet"}
                  </p>
                </div>
                {c.unread_count > 0 && (
